...
(function (global) {
  ...
  function roadsNear(i, j, maxNew) {
    ...
    if ((i === -16 && j === -19) || (i === -14 && j === -19)) {
      var dbgE = edges.filter(function(e){ return e.rkey === '-14_-19_0|-16_-19_0'; });
      console.error('[dbg] cell(' + i + ',' + j + ') pass 需求边=' + edges.length + ' 含目标对=' + dbgE.length);
    }
    ...
    for (var e = 0; e < edges.length; e++) {
      ...
      if (!road) {
        ...
        var path = bfsRoad(pA.q, pA.r, pB.q, pB.r, roadTileIdx);
        if (rkey === '-14_-19_0|-16_-19_0') {
          var stepsD = path ? path.length - 1 : -1, d0D = hexDist(a.q, a.r, b.q, b.r);
          var passDI = path && stepsD * 10 <= 14 * d0D;
          console.error('[dbg] 尝试 ' + rkey + ': path=' + (path ? stepsD + '步' : 'null') + ' hexDist=' + d0D + ' DI=' + (path ? (stepsD * 10 / d0D).toFixed(2) : '-') + ' 过闸=' + passDI + ' budgetIn=' + (budget + 1));
        }
        if (!path) {
