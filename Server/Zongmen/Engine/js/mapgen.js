...
(function (global) {
  ...
  var roadVer = 0;
  /* DI 重试队列节流游标: 记录上次「消化一条老边」时的 roadVer。
     重试的价值完全来自「路网变密」, 所以每建成 128 条新路才值得试 1 条老边 (见 roadsNear 末)。

     实测 (±14 区域格冷启动 = 841 格, mapgen.js 单线程):
       · 每次调用都消化 1 条 → drain 烧 216 次 A* (均 27.9ms, 主循环的 2.3 倍 —— FIFO
         取出的多为大跨度老边), 只多建 7 条路, 且单区域峰值从 239ms 抬到 424ms;
       · 每 128 条新路消化 1 条 → 总耗时 20015ms (改动前 34542ms, -42%),
         峰值 239.1ms (改动前 249.0ms)。
     ⇒ 阈值取 128: 既保住「路网变密后跨调用重试」的语义, 又让代价落到噪声以下。 */
  var drainMark = 0;
  ...
  function roadsNear(i, j, maxNew, cq, cr) {
    ...
    /* DI 重试队列消化 —— 位置从「主循环之前」移到「主循环 + 延迟重试轮之后」,
       且按 roadVer 节流 (每建成 128 条新路才试 1 条老边), 不再与主循环抢预算。

       为什么必须让位: drain 里的边几乎全是「闸拒边」—— 因为已建路只收 ROAD_W_ROAD(2) 费,
       寻路总能骑着旧路绕出一条路来 (所以永远不会进「!qp 终身拉黑」分支), 但那条路永远
       步数/六边距 > 1.4, 于是被原样推回队列。原先只 push 不记账 ⇒ 队列自我补充、永不排空,
       每次调用先在队首烧掉 8 条 A*。小预算 (预览页默认 1) 下等于每次先交过路费,
       「当前视野该修的路」彻底排不上号。

       三处配套: ① 闸拒边与寻路失败同账, 3 次即终身 roadFail ⇒ 队列有终止条件;
                 ② 消化让到主循环之后 ⇒ 当前视野的路先吃满预算;
